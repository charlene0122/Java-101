...
public class ExpenseFileParser {
	...
	public static List<Map<Integer, Double>> parseExpenses(List<String> expenseList) {
		...
		// Return list of hashmaps
		 ...
		 List<Map<Integer, Double>> output = new ArrayList<>();
		    
		    // iterate over each expense line
		    for (int i = 0; i < expenseList.size(); i++)
		    {
		    	// split the line into an array using the delimiters ",: "
		        String[] line = expenseList.get(i).split("[,: ]+");
		        
		        // create a map with the month number and expense amount
		        Map<Integer, Double> expenses = new HashMap<>();
		        expenses.put(Integer.parseInt(line[0]),  Double.parseDouble(line[1]));
		        
		        // add the map to the list of expenses
		        output.add(expenses);
		    }
